...
lots of comments, which helpbepogive a false sense of order and quality.
...
Emacs 31 claims to enable ~xterm-mouse-mode~ by default, but only in terminals
it recognizes as xterm-compatible — and Doom's =:os tty= module stopped enabling
it on Emacs 31+ for that very reason. WezTerm sets =TERM=wezterm=, which slips
through both checks, leaving mouse clicks unable to move the cursor. Enable it
explicitly.
...
~kanagawa-themes~ defines ~org-hide~ as inheriting from ~org-indent~, while Org's
built-in ~org-indent~ inherits from ~org-hide~. That mutual reference is an
inheritance cycle and raises /Face inheritance results in inheritance cycle:
org-indent/ whenever the theme loads. We break it on the always-highest-priority
~user~ face layer (the theme's own colours still come through underneath).
...
We'll setup some font stuff here too. But the ~fontaine~ package is used to setup most of it (or not ~fontaine~ is currently  MIA).
...
|--------------------------+----------------------------------+----------------------|
| =SPC D=                    | ~+dashboard/open~             | Open the dashboard   |
|--------------------------+----------------------------------+----------------------|
| _+dashboard-mode-map_ |                                  |                      |
|                          |                                  |                      |
| =f=                        | ~find-file~                        | Find file            |
| =r=                        | ~consult-recent-file~              | Recent files         |
| =R=                        | ~doom/quickload-session~           | Restore last session |
...
the ~nano-modeline~.
Also we'll show the relative path instead of just filename in doom-modeline.
...
Courtesy of [[https://github.com/minad/consult/issues/318#issuecomment-882067919][noctuid ]].
...
All the =,= prefix bindings (avy above, embrace, puni's =, d=…) fight with
~evil-snipe~: after every =f=/=t=/=s= motion, snipe installs a /transient/ keymap
binding =,= and =;= to repeat the last snipe, so the first =,= pressed right
after such a motion gets swallowed and the prefix only registers on the second
try. Snipe also overrides evil's =;=/=,= repeat keys permanently in its override
map. Reclaim =,= for our prefix — =;= still repeats snipes.
...
The first time a file or a ~dired~ buffer is opened — switching project with
=SPC p p= for instance — ~doom-first-file-hook~ turns on ~envrc-global-mode~,
which walks a snapshot of ~(buffer-list)~ and enables ~envrc-mode~ in every
buffer. Since ~envrc-async~ is nil by default, that walk blocks in ~sleep-for~
while ~direnv~ evaluates the environment, which is slow with =nix=. Timers do
run during that sleep, and Doom's popup module schedules some that kill buffers
(~+popup--kill-buffer~), so a buffer from the snapshot can die mid-walk and the
next ~with-current-buffer~ signals /Selecting deleted buffer/. It only ever
happens once per session, because ~doom-first-file-hook~ only runs once.

Running ~direnv~ asynchronously for that initial walk keeps it from sleeping, so
no timer gets the chance to pull a buffer out from under it. Buffers opened
afterwards still take the blocking path, so mode hooks keep seeing the
...
The =docker= CLI defaults to =/var/run/docker.sock=, which here is a symlink to
podman's /rootful/ socket. Images pulled as my own user live in the /rootless/
store instead, so =SPC o D= listed nothing while =podman images= in a shell
showed everything: two daemons, two separate stores. ~docker.el~ shells out to
=docker=, so it was faithfully reporting an empty store.

Pointing ~DOCKER_HOST~ at the user socket makes ~docker.el~ talk to the same
podman the shell does. Guarded twice, so it never overrides an explicit
~DOCKER_HOST~ and never fires on a machine that has a real rootful daemon and no
user socket.
...
Display ~hl-todo-keywords~ in ~org-mode~, such as FIXME/TODO/BUG/REVIEW…
...
I like the default colored bars, lets reset the values.

Restoring ~indent-bars-highlight-current-depth~ re-enables the current-depth
highlight, which Doom turns off — and that puts
~indent-bars--update-current-depth-highlight~ on ~post-command-hook~, so moving
point schedules an idle timer holding the current buffer.

If the buffer's major mode changes before that timer fires,
~kill-all-local-variables~ wipes ~indent-bars-mode~, ~indent-bars-spacing~,
~indent-bars--offset~ /and/ the buffer-local timer handle. ~indent-bars-teardown~
then has nothing left to cancel the timer with, so it fires anyway, into a
buffer whose ~indent-bars--offset~ is now nil:

: Error running timer 'indent-bars--update-current-depth-highlight-in-buffer': (wrong-type-argument number-or-marker-p nil)

The timer callback is missing the ~(when indent-bars-mode …)~ guard that its own
scheduler has, so give it one. Upstream bug — latent for everyone else because
Doom disables this highlight in the first place.
...
** ~nov~ · « 'Cause nothin' lasts forever, even cold ~Nov~ ember rain » 󰎇
...
TODO: adopt ~newspeak.el~ for *everything*.
...
~YAML~ /is/ indentation, and yet nothing here lets me move by it: no defuns means
~[[~ and ~]]~ have nothing to chew on, and ~imenu~ only ever sees column zero. So let's
teach it the four directions already under my fingers — ~t~ up, ~r~ down, ~l~ out, ~i~ in.
...
Now the moves. ~evil~ motions rather than plain commands, so counts and operators
come for free: ~3gr~ skips three siblings, ~dgr~ deletes linewise through the next
one, ~vgrgr~ grows a selection sibling by sibling.
...
And the mapping — in ~YAML~ buffers only, so the collateral damage is contained:
~+eval:region~ (evaluating ~YAML~? no), ~evil-tab-next~ (workspaces live on ~g«~ / ~g»~),
~evil-insert-resume~, and the only one that stings, ~evil-lion-left~. Drop the ~g l~
line if aligning colons matters more than moving out a level.
...
Indentation is the /only/ thing this reads, so it survives a later switch to
~yaml-ts-mode~. That mode brings its own version of these four moves on ~C-M-n~,
~C-M-p~, ~C-M-u~ and ~C-M-d~, via a ~treesit~ ~list~ thing over ~block_mapping_pair~ — but
those aren't motions, so no counts and no operators.

*** Focusing on a block

Reading =yaml= is easier than reading =json=, which doesn't mean that I'm not getting lost in all the indentations.
But NO MORE. ~zi~ focus in the "bloc" under the cursor. ~zu~ to undo.
...
~zi~ to dive in, ~zu~ to come back up one level at a time — both were free, and
~zn~ / ~zN~ keep doing their generic thing.
...
*** A ~vertico~-shaped ~imenu~

~SPC s i~ and ~gO~ already route through ~consult-imenu~, so the machinery is fine — the
/index/ is the problem. ~yaml-mode~ builds it from a regexp anchored at column zero,
which on a ~Deployment~ means four entries: ~apiVersion~, ~kind~, ~metadata~ and ~spec~.
Every key I actually want to reach is nested.

So let's index every key and name it by its path. ~containers~ becomes
~spec / template / spec / containers~, and since ~orderless~ doesn't care about order,
« /cont temp/ » is enough to land on it.
...
Only for ~yaml-mode~: ~yaml-ts-mode~ already gets a nested index out of ~treesit~.
...
Nesting comes from the /column of the key/, not the indentation of the line, so
~- name:~ and the ~image:~ under it come out as siblings instead of parent and
child. Multi-document files just repeat their top-level keys, which is honest.
~SPC s I~ spreads the same index across every open buffer.
...
|--------------------------------------+--------+---------|
** ~occur~ · Search & Replace

Search & replace, the bread and butter of coding/refactoring.

Emacs is king in this area, but I +sometimes+ often suffer brain damage and forget
how to Emacs.
...
/Given a project containing multiples files. You're trying to search and replace
every word '_vim_' and replace it with '_Emacs_'./

1. =SPC /=        -> *Make a global search in the project with ~+default/search-project~.*
2. *input 'vim' without pressing =RET=.*
3. =C-,=          -> *invoke ~embark-act~.*
4. =E=            -> *invoke ~embark-export~.*
   /This will create a buffer with all matching results./
5. =C-c C-p=      -> *invoke ~wgrep-change-to-wgrep-mode~.*
   /This allows us to modify the buffer. Then./
6. =Z Z=, =C-c C-c= -> *invoke ~wgrep-finish-edit~.*
   /The magic is done, you've successfully completed a search and replace./
...
|---------+-------------------------------+---------|
-----
** Showing unsaved changes in the current buffer

Sometimes, I edit a file, but I can't remember what I changed, should I keep it
or discard it ? =M-x diff-buffer-with-file= to the rescue.
...
/Emacs Lisp code. Then, when we are satisfied with the potency of the experiment,
it will be moved to the relevant level./
